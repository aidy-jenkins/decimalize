...
const number = pz.terminal(ExprToken.Number, "<number>").map(token => new BigNumber(token.text)).followedBy(optws);
...
const numberFromArg = (arg: string) => parseInt(arg.substring(1, arg.length - 1)); //{123} -> 123
...
module Decimalize {
    ...
    export function combineElements(strings: TemplateStringsArray | string[], ...elements: string[]) {
    ...
    export function evaluate(expression: string, inputs: ExpressionElement[]) {
        ...
        inputFromArg = arg => new BigNumber(inputs[numberFromArg(arg)] as number | BigNumber | string);
